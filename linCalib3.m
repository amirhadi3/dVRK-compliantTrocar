...
Ls_offset_min = 0.9*0.400;
Ls_offset_max = 1.1*0.400;

if fit_active
    lb = [-inf*ones(6*m,1);0.8*L;0.2*cx;0.2*cx;Lp;Ls_offset_min;-inf;-inf];
    ub = [ inf*ones(6*m,1);1.2*L; 10*cx;10*cx;Lp;Ls_offset_max;inf;inf];
    
    f = @(x) upNonLinModel3(x,possig,Nsig,atiN,m,maxati);
    ip = [rand(6*m,1);L;cx;cx;Lp;Ls_offset_max;rand(2,1)];
    % load('linear_fit-swarmLowestFunVal.mat')
    % ip = optimal_x(1:6*m+5);
    options = optimoptions(@lsqnonlin,'MaxIterations',1000,'Algorithm',...
        'trust-region-reflective','Display','iter');
    % [optimal_x,resnorm] = lsqnonlin(f,ip,lb,ub,options);
    % pswarmopt = optimoptions('particleswarm','SwarmSize',200,'HybridFcn',@fmincon,'MaxIterations',1000,'Display','iter','UseParallel',true,'UseVectorized', false);%,'MaxFunctionEvaluations',inf);
    ...
    load('linear_fit.mat')
    optimal_x(6*m+3) = optimal_x(6*m+2);
end
...
H(:,2) = -(1-2*Ls.*gx);
H(:,3) = 0;
H(:,4) = -((Ls-Lp)-2*Ls.*(Ls-Lp).*gy);
H(:,5) = (Ls-Lp)-2*Ls.*(Ls-Lp).*gx;
H(:,6) = 0;

csf = csf-H*diag([Fsx;Fsy;0;Fsx;Fsy;0]);
...
frec(:,3) = csf(:,3);
frec(:,2) = (1./det2).*(F44.*csf(:,2)-F24.*csf(:,4));
frec(:,4) = (1./det2).*(-F42.*csf(:,2)+F22.*csf(:,4));
frec(:,6) = csf(:,6);
%%
os = 1*Fs;
