...
class psm_random_move(object):
    ...
    def home(self):
        ...
        for item in self.home_pos:
            ...
            axis+=1
	self.arm.move_jaw(0,interpolate = True, blocking = True)
...
if __name__ == "__main__":
	...
	motion_rng = [[-0.02,0.02],[-math.pi/4,math.pi/4],[-math.pi/6,math.pi/6],[-math.pi/6,math.pi/6]]
	
	p = psm_random_move(armname,5,4,axisIndex,motion_rng,0.001)
	p.home()

	traj = p.traj_generate()

	# generate jaw motion
	jp = p.arm.get_current_jaw_position()
	jgrip = p.singleAxisTrajGen(0,0,p.num_wayPoints,[-math.pi*30/180,math.pi*30/180]) + jp
	if False:
	    plt.plot(jgrip)
	    plt.show()
	...
	insertion = np.arange(0.075,0.19,0.2)
	...
	for ins in insertion:
	    ...
	    curPos = p.arm.get_current_joint_position()
	    cmd = curPos[axisIndex]+traj;

	    dpoints = min(len(cmd[:,1]),len(jgrip))
            for index in range(dpoints):
            	p.arm.move_joint_some(cmd[index,:],axisIndex,interpolate = False, blocking = False)
