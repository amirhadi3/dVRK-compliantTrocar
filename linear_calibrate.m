...
warning('on','all');
addpath(sprintf('%s%s',pwd,'/20201102-3'));
fit_active = true;
print_on = false;
plot_suffix = '_linear';
%% read data
...
if fit_active
    ...
    % % options = optimoptions(@fmincon,'MaxIterations',1000,'Display','iter','MaxFunctionEvaluations',inf,'FunctionTolerance',1e-6);
    % % [optimal_x,resnorm] = fmincon(f,ip,[],[],[],[],lb,ub,[],options);
    ...
    [optimal_x,errormulti] = run(ms,problem,100);
